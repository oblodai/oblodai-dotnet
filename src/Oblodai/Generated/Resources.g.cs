...
public sealed partial class Refunds : Resource
{
    ...
    /// <para>`address` (where to refund) may be omitted ONLY if the payment has `payer_address_is_refundable` = true: then we refund to the recorded payer address (`payer_address`). If it is false, we know the payer's address but it is not a refund address (Bitcoin/UTXO: the first input may belong to an exchange or be change; XRP: a shared exchange address with a destination tag; CARD payment via a crypto on-ramp: the sender is the provider's omnibus hot wallet, not the buyer). A refund sent there is irrecoverably lost to someone who never paid, so a request without `address` is rejected (`refund.no_address`): ask the buyer for an address and pass it explicitly. The payment's `uuid`/`order_id` is required. By default the remaining refundable amount is refunded; you may specify a partial `amount`. All refunds of a payment together cannot exceed its `refundable` amount: what was paid minus the payer's network surcharge (minus our commission when the store's refund fee setting puts it on the customer), never more than was credited to your balance for it — POST /v1/payment/refund/calculate shows these numbers without refunding.</para>
    ...
    /// <param name="amount">The amount to refund, in the payment coin. Without it the refund is what is still refundable: the amount paid minus the payer's network surcharge and — when the store's refund fee setting (getRefundFeeConfig) puts the commission on the customer — minus the Oblodai commission too, never more than was credited to your balance for this payment, less the refunds already made. All refunds of a payment together cannot exceed that refundable amount (refund.exceeds_refundable); POST /v1/payment/refund/calculate shows it.</param>
    ...
    /// <remarks>
    /// <para>Takes the same body as POST /v1/payment/refund and answers what that refund would send — `amount`, `currency`, `network`, `address` (and whether it is the recorded payer's) — and the numbers behind it: `amount_paid`, the payer's network `surcharge` (never refunded from your balance), the `commission` withheld and who bears it (`commission_bearer`, the store's refund fee setting), `credited`, the `refundable` ceiling for all refunds of the payment together, what is already `refunded` and what `remaining` can still go. With `from_currency` it also estimates the USDT the funding conversion would spend (`from_amount`).</para>
    /// <para>Runs the same checks as the refund itself and fails with the same error the refund would (`refund.exceeds_refundable`, `refund.dust`, `refund.no_address`, `refund.nothing_to_refund`, …) — except the destination address screening, which runs when the refund is made. Reserves and sends nothing; safe to retry.</para>
    /// <para>Requires role: Viewer when called with a CLI key.</para>
    /// <para>Errors: <c>auth.bad_timestamp</c>, <c>auth.body_too_large</c>, <c>auth.ip_not_allowed</c>, <c>cli.permission_denied</c>, <c>internal</c>, <c>invoice.corrupt_pay_asset</c>, <c>merchant.bad_signature</c>, <c>merchant.key_expired</c>, <c>merchant.key_mode_mismatch</c>, <c>merchant.rate_limited</c>, <c>merchant.secret_decrypt</c>, <c>merchant.suspended</c>, <c>merchant.unknown_key</c>, <c>onramp.suppresses</c>, <c>payment.bad_uuid</c>, <c>payment.no_lookup</c>, <c>payment.not_found</c>, <c>payout.above_limit</c>, <c>payout.address_network_mismatch</c>, <c>payout.bad_address</c>, <c>payout.bad_memo</c>, <c>payout.cap_unpriceable</c>, <c>payout.convert_bad_amount</c>, <c>payout.convert_no_rate</c>, <c>payout.convert_same_asset</c>, <c>payout.convert_unsupported</c>, <c>payout.daily_cap</c>, <c>payout.freeze_unknown</c>, <c>payout.frozen</c>, <c>payout.memo_conflict</c>, <c>payout.memo_required</c>, <c>payout.memo_too_long</c>, <c>payout.merchant_frozen</c>, <c>rates.deviation</c>, <c>rates.no_source</c>, <c>rates.non_positive</c>, <c>rates.stale_rate</c>, <c>refund.bad_amount</c>, <c>refund.chain_ambiguous</c>, <c>refund.destination_internal</c>, <c>refund.dust</c>, <c>refund.exceeds_refundable</c>, <c>refund.fence_check</c>, <c>refund.from_currency_personal_account</c>, <c>refund.from_currency_unsupported</c>, <c>refund.network_required</c>, <c>refund.no_address</c>, <c>refund.nothing_to_refund</c>, <c>refund.omnibus_destination</c>, <c>refund.paid_internally</c>, <c>refund.unsupported_network</c>, <c>request.bad_json</c>, <c>request.body_read</c>, <c>request.control_char</c>, <c>request.duplicate_field</c>, <c>request.nul_byte</c>, <c>request.overloaded</c>, <c>request.rate_limited</c>, <c>request.too_deep</c>, <c>sandbox.convert_not_available</c>, <c>treasury.no_ccy_map</c>, <c>wallet.static_not_found</c>.</para>
    /// </remarks>
    /// <param name="request">The request body.</param>
    /// <param name="options">Per-call options: idempotency key, timeout, retries, extra headers, request id.</param>
    /// <param name="cancellationToken">Cancels the call.</param>
